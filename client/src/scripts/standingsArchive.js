...
import StandingsArchiveABI from "./StandingsArchiveABI.json";
...
const standingsArchiveContract = new ethers.Contract(
  contractAddress,
  StandingsArchiveABI,
  provider
);
...
async function getAllStandings() {
  try {
    ...
    const standings = await standingsArchiveContract.getAllStandings();
    // Return the articles
    return standings;
  } catch (error) {
    // Log the error and return an empty array
    console.error("Error fetchingstandings:", error);
    return [];
  }
}
